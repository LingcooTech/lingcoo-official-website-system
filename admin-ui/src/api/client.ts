...
export interface Inquiry {
  ...
  assignee: { id: string; email: string | null; displayName: string | null } | null;
...
export async function fetchInquiries(input: {
  page: number;
  pageSize: number;
  status?: InquiryStatus;
  search?: string;
  ...
  return apiRequest(`/api/inquiries?${params}`);
}
...
export async function updateInquiry(
  ...
  return (
    await apiRequest<{ inquiry: Inquiry }>(`/api/inquiries/${inquiryId}`, {
      method: 'PATCH',
      body: JSON.stringify(input),
    })
  ).inquiry;
...
export async function fetchInquiryAssignees(): Promise<InquiryAssignee[]> {
  return (await apiRequest<{ items: InquiryAssignee[] }>('/api/inquiries/assignees')).items;
}
