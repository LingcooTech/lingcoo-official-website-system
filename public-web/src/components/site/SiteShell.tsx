...
export function SiteBrand({
  ...
  const displayName = presentation?.displayName ?? 'Lingcoo';
  ...
  return (
    <a className="site-brand" href="/" aria-label={`${displayName} 首页`}>
      ...
      <span className="site-brand__name">
        ...
        <small>{presentation?.slogan ?? '让复杂系统，清晰生长'}</small>
      </span>
    </a>
  );
}
      ...
      <Container>
        <div className="public-site-footer__main">
          <div>
            ...
            <p>{presentation?.slogan ?? '让复杂系统，清晰生长'}</p>
          </div>
          {footerLinks.length ? (
            <nav aria-label="页脚导航">
        ...
        <div className="public-site-footer__legal">
          <span>
            {presentation?.footerCopyright ??
              `© ${new Date().getFullYear()} ${presentation?.displayName ?? 'Lingcoo'}`}
          </span>
